...
import { PlusCircle } from 'lucide-react'; // Both buttons will use PlusCircle
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-background border-t border-border shadow-lg z-40 flex justify-around items-center h-16 px-2 sm:px-4">
      ...
      <Link
        href="/browse"
        className={cn(
          'flex flex-col items-center text-center transition-colors pt-1', // Added pt-1 for slight adjustment if needed
      ...
      >
        ...
        <div className="relative -mt-8 mb-1 flex h-[60px] w-[60px] items-center justify-center rounded-full bg-primary text-primary-foreground shadow-lg transition-all duration-150 ease-out hover:bg-primary/90 active:scale-95 hover:shadow-xl">
          <PlusCircle className="h-7 w-7" /> {/* User wants PlusCircle for Acheter/Parcourir */}
        </div>
      ...
      <Link
        href="/sell"
        className={cn(
          'flex flex-col items-center text-center transition-colors pt-1', // Added pt-1 for slight adjustment
